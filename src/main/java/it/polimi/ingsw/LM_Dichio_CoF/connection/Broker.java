...
public class Broker {
	...
	public static void login(Player player, GameSide gameSide){
		...
		if(player.getTypeOfConnection()=='s'){
			...
			while(!logged){
				...
				if(!isNicknameInUse(nickname,gameSide)){
	...
	public static boolean isNicknameInUse(String nickname,GameSide gameSide){
		for(Player player: gameSide.getArrayListPlayer()){
	...
	public static synchronized void sayMatchHasStarted(Player player){
		if(player.getTypeOfConnection()=='s'){
			sendString("SOCKETplayMatch",player);
		}else{
			try {
